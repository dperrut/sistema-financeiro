...
export default function GoalsView({ 
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8">
      ...
      {/* FORMULÁRIO DE NOVA META REFINADO (Modo Escuro) */}
      <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-sm border border-purple-100 dark:border-purple-900/50 transition-colors duration-300">
        <h3 className="text-xs font-bold text-purple-800 dark:text-purple-300 mb-2 flex items-center gap-2">
          <Target size={14} className="text-purple-600 dark:text-purple-400"/> Crie sua próxima meta
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-11 gap-2">
          ...
          <div className="md:col-span-2 relative">
            <label className="absolute -top-2 left-2 bg-white dark:bg-gray-800 px-1 text-[9px] font-bold text-purple-400 dark:text-purple-300 transition-colors">Data para Resgate</label>
            <input 
              type="date" 
              className="w-full p-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 dark:text-white outline-none focus:border-purple-400 cursor-pointer transition-colors" 
          ...
          <div className="md:col-span-2">
            <button 
              onClick={addGoal} 
              className="w-full bg-purple-600 dark:bg-purple-700 text-white p-2 text-sm rounded-lg font-bold hover:bg-purple-700 dark:hover:bg-purple-600 shadow-sm transition-all active:scale-95"
            >
              Criar Meta
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        {goals.map(g => {
          ...
          return (
            <div key={g.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-purple-100 dark:border-purple-900/30 flex flex-col justify-between h-full hover:shadow-md transition-all duration-300">
              <div className="space-y-3">
                {/* Título e Sugestão */}
                <div>
                  <h4 className="font-bold text-gray-800 dark:text-gray-100 text-sm leading-tight truncate" title={g.name}>{g.name}</h4>
                  
                  {mensalSugestao > 0 && percent < 100 ? (
                    <div className="mt-1 bg-purple-50 dark:bg-gray-700 p-1.5 rounded-lg border border-purple-100 dark:border-gray-600 transition-colors">
                ...
                <div className="space-y-1">
                  ...
                  <div className="w-full bg-gray-100 rounded-full h-2 overflow-hidden mt-2">
                    <div className="bg-purple-500 dark:bg-purple-400 h-2 rounded-full transition-all duration-1000" style={{width: `${percent}%`}}></div>
                  </div>
                ...
                {g.targetDate && (
                  <div className="bg-gray-50 dark:bg-gray-700 p-1.5 rounded-md border border-gray-100 dark:border-gray-600 text-center transition-colors">
                  <p className="text-[9px] text-gray-400 dark:text-gray-400 font-bold uppercase">Previsão de Resgate</p>
                  <p className="text-[11px] font-bold text-gray-600 dark:text-white">{g.targetDate.split('-').reverse().join('/')}</p>
                  </div>
                )}
              </div>
