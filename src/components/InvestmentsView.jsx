...
export default function InvestmentsView({ 
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 p-3 rounded-xl shadow-sm border border-blue-100 dark:border-blue-900/50 border-l-4 border-l-blue-500 transition-colors duration-300">
        <h3 className="text-xs font-bold text-blue-800 dark:text-blue-300 mb-2 flex items-center gap-2">
          <TrendingUp size={14} className="text-blue-600 dark:text-blue-400"/> Iniciar Novo Investimento
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-12 gap-2">
          <div className="md:col-span-6">
            <input 
              className="w-full p-2 text-sm border border-gray-200 dark:border-gray-600 rounded-lg bg-gray-50 dark:bg-gray-700 dark:text-white outline-none focus:border-blue-400 transition-colors" 
          ...
          <div className="md:col-span-2">
            <button 
              ...
              className="w-full bg-blue-600 dark:bg-blue-700 text-white p-2 text-sm rounded-lg font-bold hover:bg-blue-700 dark:hover:bg-blue-600 transition-all active:scale-95 shadow-sm"
            >
              Criar Invest.
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
        {investments && investments.map(inv => (
          <div key={inv.id} className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-blue-50 dark:border-blue-900/30 flex flex-col justify-between h-full hover:shadow-md transition-all duration-300">
            <div className="space-y-4">
              <div className="flex justify-between items-start">
                <div className="max-w-[80%]">
                  <h4 className="font-bold text-gray-800 dark:text-gray-100 text-sm leading-tight truncate" title={inv.name}>{inv.name}</h4>
                  <span className="text-[9px] bg-blue-100 dark:bg-blue-900/20 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded-full font-bold uppercase mt-1 inline-block">Ativo Financeiro</span>
                </div>
            ...
            <div className="pt-3 mt-4 border-t border-gray-50 space-y-1.5">
              ...
              <button onClick={()=>deleteInvestment(inv.id)} className="w-full text-[10px] text-gray-400 dark:text-gray-500 py-1 font-bold hover:text-red-500 dark:hover:text-red-400 transition-colors rounded-md">
